...
function main(argv) {
  ...
  `)
    ...
    const scriptPath = args._.length
        ? path_1.resolve(cwd, args._[0])
        : undefined
        ...
        if (args._.length) {
            ...
            if (interactive || process.stdin.isTTY) {
                ...
                process.stdin.on('end', () =>
                    evalAndExit(service, state, module, buffer, print)
                )
            }
        }
    }
...
function getCwd(dir, scriptMode, scriptPath) {
    ...
    if (scriptMode) {
        if (!scriptPath) {
            throw new TypeError(
                'Script mode must be used with a script name, e.g. `ts-node -s <script.ts>`'
            )
        }
        if (dir) {
...
function evalAndExit(service, state, module, code, isPrinted) {
    ...
    if (isPrinted) {
        console.log(
            typeof result === 'string' ? result : util_1.inspect(result)
        )
    }
}
/**
...
function startRepl(service, state, code) {
    ...
    if (repl.setupHistory) {
        const historyPath =
            process.env.TS_NODE_HISTORY ||
            path_1.join(os_1.homedir(), '.ts_node_repl_history')
